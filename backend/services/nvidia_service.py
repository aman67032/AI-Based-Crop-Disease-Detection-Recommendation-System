"""
NVIDIA API service — Primary AI provider.
Uses OpenAI-compatible SDK pointed at NVIDIA's endpoint.
Supports both text recommendations and vision (Llama 4 Maverick is multimodal).
"""
...
async def get_recommendation(
    ...
    """Generate treatment recommendation using NVIDIA API."""
    lang_instruction = {
...
async def analyze_image_with_nvidia(image_bytes: bytes, crop_hint: str = "") -> str:
    """Send leaf image to NVIDIA (Llama 4 Maverick) for visual disease analysis."""
    ...
    try:
        ...
        if not result:
            raise ValueError("NVIDIA returned empty response")
        return result
    except Exception as e:
